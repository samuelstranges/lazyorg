...
func (kbv *KeybindsView) Update(g *gocui.Gui) error {
	...
	fmt.Fprintln(v, "")
	fmt.Fprintln(v, " Goto:")
	fmt.Fprintln(v, " g + t        - Goto time (HH format)")
	fmt.Fprintln(v, " g + d        - Goto date (YYYYMMDD format)")
	fmt.Fprintln(v, "")
	...
	fmt.Fprintln(v, " /            - Search events in week (name/desc/location)")
	...
	fmt.Fprintln(v, " Esc          - Cancel color selection")
	fmt.Fprintln(v, "")
