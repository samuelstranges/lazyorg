...
been doing a little vibecoding on this project... added:

- colored events
- undo and with `u` and redo with `r`
- yank and paste events (`y` and `p`, `d` also adds to buffer)
- jumps with `g`
- search with `/` within current week
- prev/next events within week with `w` and `b`

## bugs

- events on sunday act funny
...
Switch **Your/location** to your current location. Use
`timedatectl list-timezones` to fetch a list of possible locations.
...
Download pre-compiled binary from the latest release. MacOS and Windows have not
been tested yet.
...
When creating a new event (`a`), you'll be prompted to fill in the following
fields:
...
Configuration file will come in future releases. For now, when you open the app
for the first time, the database is created at `~/.local/share/lazyorg/data.db`
...
- Thanks to _defnotgustavom_ for the
  [docker image](https://hub.docker.com/r/defnotgustavom/lazyorg)
